...
FindPlacesGoogleInput
        ...
        │       Передаёт: type, radius, opennow
        │       maxPlaces = 60 (захардкожено)
        │       → GooglePlace[]  (rawPlaces)
        │
        ├─► filterPlaces(rawPlaces, { minReviewCount })   [filter.ts]
        │
        │       → PlaceData[]  (filtered)
        │
        ...
        │
        ├─► apifyReviewScraper({ placeIds, limit: maxReviewsPerPlace })
...
```ts
filterPlaces(rawPlaces, { minReviewCount });
```
